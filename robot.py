    ...
    if (station_pos[1] > 0 and last_robot_pos[1] < 0) or (station_pos[1] < 0 and last_robot_pos[1] > 0):
        ...
        if last_robot_pos[0] > -3.3 and last_robot_pos[0] < 3.3:
            if last_robot_pos[1] > -0.5 and last_robot_pos[1] < 0.5:
                ...
                waypoint1 = [math.copysign(4.8, last_robot_pos[0]), math.copysign(0.7, last_robot_pos[1])]
                waypoint2 = [math.copysign(4.8, last_robot_pos[0]), math.copysign(0.7, -last_robot_pos[1])]
                ...
                go_to_waypoint(waypoint1, exit_distance=0.4)
                go_to_waypoint(waypoint2, exit_distance=0.4)
...
def go_to_station_exceptions(stationcode, prev_stationcode):
    ...
    led(LED.RIGHT_RED, True)

    if matches_station(StationCode.BG, StationCode.EY):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        # extra careful to avoid the obstacle
        go_to_waypoint(mirror_coords([-3.3, -1.0]))

    elif matches_station(StationCode.FL, StationCode.EY):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        # extra careful to avoid the obstacle
        go_to_waypoint(mirror_coords([-2.2, -1.45]), 1.0)

    elif matches_station(StationCode.EY, StationCode.PN):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        # extra careful to avoid the obstacle
        go_to_waypoint(mirror_coords([-3.2, -2.4]), 1.0)

    elif matches_station(StationCode.PN, StationCode.TH):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        if R.time() < 60:
            go_to_waypoint(mirror_coords([-5.2, -3.35]), 0.5)

    elif matches_station(StationCode.TH, StationCode.EY):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        if R.time() < 60:
            stop()
            move(-100, 0.1)
            stop()
            set_heading(mirror(80))
            go_to_waypoint(mirror_coords([-4.8, -2.9]), 0.3)

    elif matches_station(StationCode.EY, StationCode.BG):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([-3.7, -0.6]), 0.3)

    elif matches_station(StationCode.HV, StationCode.PO):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([5.2, 0.3]), 0.2)
        go_to_waypoint(mirror_coords([5.2, -0.6]), 0.2)

    elif matches_station(StationCode.YL, StationCode.SF):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        if R.time() < 60:
            go_to_waypoint(mirror_coords([5.1, -3.2]), 0.3)

    elif matches_station(StationCode.SF, StationCode.SZ):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([6.25, 0.6]), 0.3)
        if R.time() < 60:
            go_to_waypoint(mirror_coords([5.6, 3]), 0.3)

    elif matches_station(StationCode.SZ, StationCode.PO):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([5, 0.6]), 0.3)
        go_to_waypoint(mirror_coords([5, -0.6]), 0.3)

    elif matches_station(StationCode.PO, StationCode.SZ):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([5, -0.6]), 0.3)
        go_to_waypoint(mirror_coords([5, 0.6]), 0.3)

    elif matches_station(StationCode.VB, StationCode.EY):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([-5, 0.6]), 0.3)
        go_to_waypoint(mirror_coords([-5, -0.6]), 0.3)

    elif matches_station(StationCode.EY, StationCode.VB):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([-5, -0.6]), 0.3)
        go_to_waypoint(mirror_coords([-5, 0.6]), 0.3)

    elif matches_station(StationCode.VB, StationCode.OX):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([-2.6, 1.7]), 0.4)

    elif matches_station(StationCode.SZ, StationCode.BN):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([2.6, 1.7]), 0.4)

    elif matches_station(StationCode.VB, StationCode.BG):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([-3.6, 0.8]), exit_distance=0.5)

    elif matches_station(StationCode.SZ, StationCode.HV):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([3.6, 0.8]), exit_distance=0.5)

    elif matches_station(StationCode.BG, StationCode.PN):
        print(f"###########{prev_stationcode}------>{stationcode}##############################")
        go_to_waypoint(mirror_coords([-4.7, -0.9]), exit_distance=0.5)



    # chris exceptions time!

    elif matches_station(StationCode.VB, StationCode.TH):
        print("i did a thing")
        go_to_waypoint(mirror_coords([-4, 1.75]), exit_distance=1)   
...
def back_off(dest_point):
    led(LED.LEFT_RED, True)
    led(LED.RIGHT_RED, True)
    bearing = get_bearing(last_robot_pos, dest_point)
    # back off by reversing up turn approx 90. drive forward a bit and then re-orient towards dest_point
    set_power(-100, -5)
    R.sleep(0.21)
    stop()
    move(-100, 0.1)
    sweep()
    stop()
    bearing = get_absolute_bearing(last_robot_pos, dest_point)
    set_heading(bearing)
    led(LED.LEFT_RED, False)
    led(LED.RIGHT_RED, False)


def go_to_waypoint(point, exit_distance=0.3, slow_down_on_approach=False):
    led(LED.LEFT_BLUE, True)
    led(LED.RIGHT_BLUE, True)
...
def go_to_station(stationcode, prev_stationcode):
    ...
    while strength < 4.2:
        ...
        if ismine(stationcode) and stationcode in tx_status['latest']:
            stop()
            led(LED.LEFT_GREEN, False)
            led(LED.RIGHT_GREEN, False)
            return
        bearing, distance, strength = get_bearing_distance_strength(stationcode)
        print(
        ...
        if front_bumper() or not_moving():
            if front_bumper():
                ...
                print("stuck - not moving")
            # move back with random powers -50 to -100 so we can get some random turning
            back_off(station_pos_dict[stationcode])
...
def claim_station(stationcode, next_stationcode):
    def matches_station(current, next):
        return stationcode == mirror_station(current) and next_stationcode == mirror_station(next)
    ...
    if (matches_station(StationCode.SZ, StationCode.HV)):
        target_heading = mirror(170)
    elif (matches_station(StationCode.HV, StationCode.PO)):
        target_heading = mirror(90)
    else:
        ...
        if diff_station_bearing > 0:
            target_heading = add_bearing(current_station_bearing, 80)
        else:
            target_heading = add_bearing(current_station_bearing, -80)
    current_heading = get_heading()
...
led(LED.LEFT_BLUE, False)
led(LED.LEFT_RED, True)
go_to_waypoint(mirror_coords([-5.5, -3.2]), .3, False)
led(LED.LEFT_RED, False)
led(LED.LEFT_GREEN, True)
go_to_waypoint(mirror_coords([-5, -3]))
led(LED.LEFT_GREEN, False)
...
for i in range(0, len(stations)):
    ...
    print(f"@@@@ GOTO STATION {station_code} coming from {prev_station_code}")
    go_to_station(station_code, prev_station_code)
    stop()

    print(f"@@@@ CLAIM STATION {station_code}  next station {next_station_code}")
    claim_station(station_code, next_station_code)
    prev_station_code = station_code

# End game - keep ownership of the opposite bases
end_game_stations = [
    StationCode.HV,
    StationCode.YL,
    StationCode.SF,
...
while True:
    for i in range(0, len(end_game_stations)):
        station_code = mirror_station(end_game_stations[i])
        next_station_code = mirror_station(end_game_stations[(i+1) % len(end_game_stations)])
        print(f"$$$$ GOTO STATION {station_code} coming from {prev_station_code}")
        go_to_station(station_code, prev_station_code)
        stop()
        ...
        # Force a sweep because otherwise the state doesn't update
        sweep()
